...
### 8.2 エージェント役割定義とタスク種別判定
- **implementation-explorer**: 実装前の関連情報調査・既存コード分析・実装方針策定・調査報告書作成
- **coder**: implementation-explorerの調査結果を参照して実装・開発作業
- **reviewer**: コードレビューと品質確認
- **tester**: テスト実行と動作確認
- **debugger**: 問題発生時の調査と修正作業

#### タスク種別の自動判定とワークフロー
設計書の内容分析により、以下の基準でタスク種別を自動判定し、適切なエージェントワークフローを設定：

- **実装タスク**: API、UI、機能実装、TDDサイクルを含む → implementation-explorer → coder → reviewer → tester
- **調査・分析タスク**: 既存コード分析、技術調査、要件整理を含む → implementation-explorer → reviewer
- **ドキュメント作成タスク**: 仕様書、README、技術文書作成を含む → implementation-explorer → reviewer
- **設定・構築タスク**: 環境構築、設定ファイル、インフラ関連を含む → implementation-explorer → coder → reviewer
- **テスト専用タスク**: テスト実行、E2E確認、動作検証のみ → tester → reviewer
- **デバッグ専用タスク**: 不具合修正、性能改善、問題解決を含む → debugger → reviewer → tester
- **レビュー専用タスク**: コードレビュー、品質確認のみ → reviewer

### 8.3 ワークフロー制御
- 各タスクファイルで指定されたエージェントワークフローに従った動的実行
- エラー発生時のdebugger自動アサインと復旧フロー
- タスク種別に応じた最適なエージェント組み合わせの自動選択
...
## 4. このタスクのエージェントワークフロー

**タスク種別**: 実装（TDDサイクル）

**ワークフロー**:
1. implementation-explorer: 関連コード調査・実装方針策定・TDDアプローチ計画
2. coder: 実装・開発作業（Red-Green-Refactorサイクル）
3. reviewer: コードレビュー・品質確認
4. tester: テスト実行・動作確認

**条件分岐**:
- テスト失敗時: debugger → reviewer → tester
- 実装エラー時: debugger → coder → reviewer → tester
...
### 6.2 機能実装 (Green)
- **目的**: 6.1で作成したテストケースがすべて成功するように、最小限のAPIロジックを実装する。
...
- **目的**: 6.2で実装したコードの内部品質（可読性、保守性、パフォーマンス等）を、テストが成功状態を維持したまま改善する。
...
## 7. この工程の成果ファイル生成 (作業完了後に必ず実行してください)
...
**ワークフロー**:
1. implementation-explorer: 既存コンポーネント調査・実装方針策定・Playwrightテスト計画
2. coder: コンポーネント実装・スタイリング・API連携
3. reviewer: コードレビュー・品質確認
4. tester: Playwrightによる動作確認・E2Eテスト実行

**条件分岐**:
- UI表示エラー時: debugger → coder → reviewer → tester
- Playwrightテスト失敗時: debugger → reviewer → tester
...
1. **工程001**: [工程名] → `task_001_[工程名].md`
2. **工程002**: [工程名] → `task_002_[工程名].md`
3. **工程003**: [工程名] → `task_003_[工程名].md`
（...全工程をリスト表示...）
...
1. **工程の順次実行**：
   - 成果ファイル（`results/task_XXX_result.md`）が存在する工程はスキップ
   - 未完了の工程を順番に実行

2. **各工程の実行**：
   - 該当タスクファイル（`task_XXX_[工程名].md`）を読み込んで実行
   - 成果ファイルが生成されたら次の工程へ
